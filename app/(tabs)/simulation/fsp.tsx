...
function FSPSimulationScreen() {
  ...
  const cleanupVoiceflowWidget = async (
    ...
    try {
      ...
      console.log('📞 FSP: Step 3 - Ending Voiceflow conversation/voice call...');
      if (window.voiceflow?.chat) {
        try {
          ...
          if (window.voiceflow.chat.proactive?.clear) {
            ...
            console.log('✅ FSP: Proactive messages cleared');
          }
        } catch (error) {
          console.error('❌ FSP: Error ending Voiceflow conversation:', error);
        }
      ...
      console.log('🔚 FSP: Step 5 - Closing Voiceflow widget UI...');
      if (window.voiceflow?.chat) {
        try {
          window.voiceflow.chat.close && window.voiceflow.chat.close();
          window.voiceflow.chat.hide && window.voiceflow.chat.hide();
      ...
      console.log('🔌 FSP: Step 8 - Closing remaining WebRTC connections...');
      if (typeof window !== 'undefined') {
        try {
          ...
          peerConnections.forEach((pc: RTCPeerConnection) => {
            if (pc && pc.connectionState !== 'closed') {
              pc.close();
              console.log('✅ FSP: Closed RTCPeerConnection');
            }
          });
          ...
          for (const ctx of audioContexts) {
            if (ctx && ctx.state !== 'closed') {
              await ctx.close();
              console.log('✅ FSP: Closed AudioContext');
            }
          }
          (window as any).__voiceflowAudioContexts = [];
        } catch (error) {
          console.error('❌ FSP: Error closing WebRTC/Audio:', error);
        }
      }

      // Step 9: Force remove DOM elements
      console.log('🗑️ FSP: Step 9 - Force removing DOM elements...');
      ...
      if (!options.skipDatabaseUpdate && sessionToken && options.finalStatus) {
        console.log('📊 FSP: Step 10 - Updating database...');
        try {
      ...
      console.log('💾 FSP: Step 11 - Clearing simulation storage...');
